...
namespace FMHandler
{
    class HostTCPClient : IDisposable
    {
        #region Disposable
        ...
        public HostTCPClient(string Host, int Port)
        {
            QueueHost = Host;
            QueuePort = Port;
        }
        ...
        private bool Connect()
        {
            ...
            //Console.Clear();
            LogWriter.show(this, "Connecting ...");
            int timeout = 3;
            while(true)
            {
                tcpclnt.Connect(QueueHost, QueuePort);
                //tcpclnt.Connect("123.231.225.20", 7777);
                //tcpclnt.Connect("192.168.1.70", 7777);            
                LogWriter.show(this, "Connected");
                ...
                if (timeout == 0)
                {
                    LogWriter.showDEBUG(this, "Beak timeout na.");
                    return false;
                }
            }
            //return false;
...
//            int timeout = 10;
            ...
            while (true)
            {
                ...
                {
                    if ((tcpclnt == null) || (tcpclnt.Connected == false) ||
                        (tcpclnt.Client == null))
                    {
                        return null;
                    }
