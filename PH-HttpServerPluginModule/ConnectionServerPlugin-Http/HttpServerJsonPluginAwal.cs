...
namespace ConnectionServerPluginHttp
{
	...
	public class HttpServerJsonPluginAwal : BaseConnectionPlugins
	{
		#region Disposable
		...
		const string namaModule = "PH Server Module Plugin";
		...
		public HttpServerJsonPluginAwal ()
			:base(namaModule) {
		...
		public void NewConnection(Stream stream, TcpClient client){
			...
			PhClientHttpHandler HttpHandler = new PhClientHttpHandler (indexConnection, configFilePath);
			HttpHandler.Translator = new PHHttpRestJsonTranslator.PhJsonToJsonTranslator ();
			HttpHandler.onDisconnected += new PhClientHttpHandler.onDisconnectedEvent (onDisconnected);
			ConnectionList.Add (indexConnection, HttpHandler);
			((PhClientHttpHandler)(ConnectionList[indexConnection])).Start (stream, client);
			indexConnection++;
