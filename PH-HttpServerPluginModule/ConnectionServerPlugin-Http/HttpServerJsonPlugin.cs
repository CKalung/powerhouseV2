...
using PHClientHttpProtocolHandler;
...
namespace ConnectionServerPluginHttp
{
	...
	public class HttpServerJsonPlugin : BaseConnectionPlugins
	{
		#region Disposable
...
//			clientHandler.Translator = ClientTranslator;
//			clientHandler.IndexConnection = indexConnection;
//			clientHandler.ConfigFilePath = configFilePath;
//			clientHandler.onDisconnected += HandleonDisconnected;
//			ConnectionList.Add (indexConnection, clientHandler);
//			((IPhClientHandler)(ConnectionList[indexConnection])).Start (stream, client);

			PhHttpClientHandler HttpHandler = new PhHttpClientHandler (indexConnection, configFilePath);
			HttpHandler.Translator = new PHHttpRestJsonTranslator.PhJsonToJsonTranslator ();
			//HttpHandler.onDisconnected += HandleonDisconnected;
			HttpHandler.onDisconnected += new PhHttpClientHandler.onDisconnectedEvent (HandleonDisconnected);
			ConnectionList.Add (indexConnection, HttpHandler);
			((PhHttpClientHandler)(ConnectionList[indexConnection])).Start (stream, client);
		...
		void HandleonDisconnected(int indexConn){
			...
			if (ConnectionList.ContainsKey (indexConn)) {
				try{ ((PhHttpClientHandler)ConnectionList [indexConn]).Dispose (); }catch{
				}
		...
		private void CloseAllConnections(){
			foreach (DictionaryEntry aConn in ConnectionList) {
				if(aConn.Value!=null)
					((PhHttpClientHandler)aConn.Value).Dispose ();
			}
