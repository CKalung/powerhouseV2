...
{
	class TCPLeopardClient: IDisposable
    {
        #region Disposable
		...
		string leopardQueueHost = "";
		int leopardQueuePort = 0;

		public TCPLeopardClient(string LeopardQueueHost, int LeopardQueuePort)
        {
			leopardQueueHost = LeopardQueueHost;
			leopardQueuePort = LeopardQueuePort;
        }
        ...
        private bool Connect()
        {
            ...
            int timeout = 3;
			LOG_Handler.LogWriter.showDEBUG(this, "Leopard > " + leopardQueueHost+ ":" + leopardQueuePort.ToString());
            while(true)
            {
				tcpclnt.Connect(leopardQueueHost, leopardQueuePort);
...
//            int timeout = 10;
            ...
            while (true)
            {
                ...
                if ((short)(tLen - 4) == msgLen)
                {
					...
					//Console.WriteLine("Terima Ti leopard = " + sb);
