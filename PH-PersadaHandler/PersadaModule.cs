...
{    
    public class PersadaModule: IDisposable
    {
        #region Disposable
        ...
        public PersadaModule(PublicSettings.Settings CommonSettings)
        {
			...
			modulus = conv.StringToBytes(commonSettings.getString("Persada_Queue_RsaKey_Modulus"));
			exponentM = conv.StringToBytes(commonSettings.getString("Persada_Queue_RsaKey_ExponentM"));

			// public & private key na client
			client_modulus = conv.StringToBytes(commonSettings.getString("Persada_Client_RsaKey_Modulus"));
			client_exponentM = conv.StringToBytes(commonSettings.getString("Persada_Client_RsaKey_ExponentM"));
			client_exponentD = conv.StringToBytes(commonSettings.getString("Persada_Client_RsaKey_ExponentD"));
		}

        public byte[] generateTransactionJson(string transactionReference, 
            string providerProductCode, long refNumSeq, ref string sJson)
        {
            string signParam = "";
            jsonH.Clear();
            string strJson = "";
